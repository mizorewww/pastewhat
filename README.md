...
- Laya 推荐项置首，其余记录维持复制时间顺序。开始选择后，异步推荐不会换掉选中的条目。
...
辅助功能权限用于读取聚焦输入框和自动发送粘贴快捷键。点击应用设置中的“开启辅助功能…”，在系统设置里允许 PasteWhat；重新打开面板即可检测。没有权限时仍能记录允许读取的剪贴板、按应用推荐、搜索和复制，然后手动 `⌘V`。
...
Laya 是结构化决策模型，不生成粘贴内容。本实现先让它判断当前语境需要的内容类型，再结合明确的输入框提示、文字/CJK/编号匹配、复制来源和较小的时间权重，在最近 20 条中选择首项。

直接让模型在 20 个编号中选择，或者为所有候选独立打“相关”概率，在调查样例中都不可靠，所以没有采用。模型类型概率也不作为“推荐正确率”展示。最终通用实现的 4 个细粒度合成场景中，邮箱、issue 链接和 staging URL 正确；`git branch` 与 `git branch -a` 的语义差异未能可靠区分。它是可用的辅助推荐，不能保证总是第一条正确。
